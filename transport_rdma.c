...
#define SMBD_TRANS(t)                                                          \
  ((struct smb_direct_transport *)container_of(t, struct smb_direct_transport, \
                                               transport))
enum { SMB_DIRECT_MSG_NEGOTIATE_REQ = 0, SMB_DIRECT_MSG_DATA_TRANSFER };
...
static struct smb_direct_recvmsg *
get_free_recvmsg(struct smb_direct_transport *t) {
  struct smb_direct_recvmsg *recvmsg = NULL;

  spin_lock(&t->recvmsg_queue_lock);
  if (!list_empty(&t->recvmsg_queue)) {
    recvmsg =
        list_first_entry(&t->recvmsg_queue, struct smb_direct_recvmsg, list);
...
static struct smb_direct_recvmsg *
get_first_reassembly(struct smb_direct_transport *t) {
  if (!list_empty(&t->reassembly_queue))
    return list_first_entry(&t->reassembly_queue, struct smb_direct_recvmsg,
                            list);
  else
    return NULL;
}
...
static void smb_direct_disconnect_rdma_work(struct work_struct *work) {
  struct smb_direct_transport *t =
      container_of(work, struct smb_direct_transport, disconnect_work);
...
static void smb_direct_send_immediate_work(struct work_struct *work) {
  struct smb_direct_transport *t =
      container_of(work, struct smb_direct_transport, send_immediate_work);
...
static struct smb_direct_transport *alloc_transport(struct rdma_cm_id *cm_id) {
  ...
  INIT_WORK(&t->post_recv_credits_work, smb_direct_post_recv_credits);
...
static void smb_direct_free_sendmsg(struct smb_direct_transport *t,
  ...
  if (msg->num_sge > 0) {
    ib_dma_unmap_single(t->cm_id->device, msg->sge[0].addr, msg->sge[0].length,
                        DMA_TO_DEVICE);
    for (i = 1; i < msg->num_sge; i++)
      ib_dma_unmap_page(t->cm_id->device, msg->sge[i].addr, msg->sge[i].length,
                        DMA_TO_DEVICE);
  }
...
static int smb_direct_check_recvmsg(struct smb_direct_recvmsg *recvmsg) {
  ...
  case SMB_DIRECT_MSG_DATA_TRANSFER: {
    ...
    struct smb2_hdr *hdr =
        (struct smb2_hdr *)(recvmsg->packet + le32_to_cpu(req->data_offset));
    ksmbd_debug(RDMA,
                "CreditGranted: %u, CreditRequested: %u, DataLength: %u, "
                "RemainingDataLength: %u, SMB: %x, Command: %u\n",
                le16_to_cpu(req->credits_granted),
                le16_to_cpu(req->credits_requested), req->data_length,
                req->remaining_data_length, hdr->ProtocolId, hdr->Command);
    break;
  }
  case SMB_DIRECT_MSG_NEGOTIATE_REQ: {
    ...
    ksmbd_debug(RDMA,
                "MinVersion: %u, MaxVersion: %u, CreditRequested: %u, "
                "MaxSendSize: %u, MaxRecvSize: %u, MaxFragmentedSize: %u\n",
                le16_to_cpu(req->min_version), le16_to_cpu(req->max_version),
    ...
    if (le16_to_cpu(req->credits_requested) <= 0 ||
        ...
        le32_to_cpu(req->max_fragmented_size) <= 128 * 1024)
      return -ECONNABORTED;
...
static void recv_done(struct ib_cq *cq, struct ib_wc *wc) {
  ...
  case SMB_DIRECT_MSG_DATA_TRANSFER: {
    ...
    t->recv_credit_target = le16_to_cpu(data_transfer->credits_requested);
    atomic_add(le16_to_cpu(data_transfer->credits_granted), &t->send_credits);

    if (le16_to_cpu(data_transfer->flags) & SMB_DIRECT_RESPONSE_REQUESTED)
...
static int smb_direct_post_recv(struct smb_direct_transport *t,
  ...
  recvmsg->sge.addr = ib_dma_map_single(t->cm_id->device, recvmsg->packet,
                                        t->max_recv_size, DMA_FROM_DEVICE);
  ...
  if (st->reassembly_data_length >= size) {
    ...
    while (data_read < size) {
      ...
      if (recvmsg->first_segment && size == 4) {
        unsigned int rfc1002_len = data_length + remaining_data_length;
        *((__be32 *)buf) = cpu_to_be32(rfc1002_len);
        data_read = 4;
        recvmsg->first_segment = false;
        ksmbd_debug(RDMA, "returning rfc1002 length %d\n", rfc1002_len);
    ...
    ksmbd_debug(RDMA,
                "returning to thread data_read=%d reassembly_data_length=%d "
                "first_entry_offset=%d\n",
                data_read, st->reassembly_data_length, st->first_entry_offset);
  read_rfc1002_done:
    return data_read;
  }
...
static void smb_direct_post_recv_credits(struct work_struct *work) {
  struct smb_direct_transport *t =
      container_of(work, struct smb_direct_transport, post_recv_credits_work);
  struct smb_direct_recvmsg *recvmsg;
...
static int smb_direct_flush_send_list(struct smb_direct_transport *t,
  ...
  first =
      list_first_entry(&send_ctx->msg_list, struct smb_direct_sendmsg, list);
  last = list_last_entry(&send_ctx->msg_list, struct smb_direct_sendmsg, list);
  ...
  if (!ret) {
    smb_direct_send_ctx_init(t, send_ctx, send_ctx->need_invalidate_rkey,
                             send_ctx->remote_key);
  } else {
    atomic_add(send_ctx->wr_cnt, &t->send_credits);
    wake_up(&t->wait_send_credits);
    list_for_each_entry_safe(first, last, &send_ctx->msg_list, list) {
...
static int calc_rw_credits(struct smb_direct_transport *t, char *buf,
                           unsigned int len) {
  return DIV_ROUND_UP(get_buf_page_count(buf, len), t->pages_per_rw_credit);
}

static int smb_direct_create_header(struct smb_direct_transport *t, int size,
                                    int remaining_data_length,
  ...
  ksmbd_debug(RDMA,
              "credits_requested=%d credits_granted=%d data_offset=%d "
  ...
  sendmsg->sge[0].addr = ib_dma_map_single(t->cm_id->device, (void *)packet,
                                           header_length, DMA_TO_DEVICE);
...
static int get_sg_list(void *buf, int size, struct scatterlist *sg_list,
                       int nentries) {
...
static int post_sendmsg(struct smb_direct_transport *t,
  ...
  for (i = 0; i < msg->num_sge; i++)
    ib_dma_sync_single_for_device(t->cm_id->device, msg->sge[i].addr,
                                  msg->sge[i].length, DMA_TO_DEVICE);
  ...
  if (send_ctx) {
    ...
    if (!list_empty(&send_ctx->msg_list)) {
      ...
      last =
          list_last_entry(&send_ctx->msg_list, struct smb_direct_sendmsg, list);
      last->wr.next = &msg->wr;
    }
...
static int smb_direct_post_send_data(struct smb_direct_transport *t,
  ...
  ret = smb_direct_create_header(t, data_length, remaining_data_length, &msg);
  if (ret) {
  ...
  for (i = 0; i < niov; i++) {
    ...
    sg_cnt =
        get_mapped_sg_list(t->cm_id->device, iov[i].iov_base, iov[i].iov_len,
    ...
    if (sg_cnt <= 0) {
      ...
      ret = -E2BIG;
      ib_dma_unmap_sg(t->cm_id->device, sg, sg_cnt, DMA_TO_DEVICE);
      goto err;
    }
...
static int smb_direct_writev(struct ksmbd_transport *t, struct kvec *iov,
                             int niovs, int buflen, bool need_invalidate,
                             unsigned int remote_key) {
  struct smb_direct_transport *st = smb_trans_direct_transfort(t);
  ...
  size_t iov_ofs;
  size_t max_iov_size =
      st->max_send_size - sizeof(struct smb_direct_data_transfer);
  int ret;
  struct smb_direct_send_ctx send_ctx;
  int error = 0;
  ...
  while (remaining_data_length) {
    ...
    while (iov_idx < niovs && possible_vecs && possible_bytes) {
      ...
      v->iov_len =
          min_t(size_t, iov[iov_idx].iov_len - iov_ofs, possible_bytes);
    ...
    ret = smb_direct_post_send_data(st, &send_ctx, vecs, nvecs,
  ...
  wait_event(st->wait_send_pending, atomic_read(&st->send_pending) == 0);
  return ret;
}
...
static void smb_direct_free_rdma_rw_msg(struct smb_direct_transport *t,
                                        ...
                                        enum dma_data_direction dir) {
  rdma_rw_ctx_destroy(&msg->rw_ctx, t->qp, t->qp->port, msg->sgt.sgl,
                      msg->sgt.nents, dir);
...
static void read_write_done(struct ib_cq *cq, struct ib_wc *wc,
                            enum dma_data_direction dir) {
  struct smb_direct_rdma_rw_msg *msg =
      container_of(wc->wr_cqe, struct smb_direct_rdma_rw_msg, cqe);
...
static int smb_direct_rdma_xmit(struct smb_direct_transport *t, void *buf,
                                int buf_len, struct smb2_buffer_desc_v1 *desc,
                                unsigned int desc_len, bool is_read) {
  ...
  for (i = 0; i < desc_num; i++) {
    msg = kzalloc(struct_size(msg, sg_list, SG_CHUNK_SIZE), KSMBD_DEFAULT_GFP);
    if (!msg) {
      ret = -ENOMEM;
      goto out;
    }
    ...
    ret =
        get_sg_list(desc_buf, desc_buf_len, msg->sgt.sgl, msg->sgt.orig_nents);
    if (ret < 0) {
      sg_free_table_chained(&msg->sgt, SG_CHUNK_SIZE);
      kfree(msg);
      goto out;
    ...
    ret = rdma_rw_ctx_init(&msg->rw_ctx, t->qp, t->qp->port, msg->sgt.sgl,
                           get_buf_page_count(desc_buf, desc_buf_len), 0,
  ...
  list_for_each_entry_reverse(msg, &msg_list, list) {
    first_wr =
        rdma_rw_ctx_wrs(&msg->rw_ctx, t->qp, t->qp->port, &msg->cqe, first_wr);
  }
...
static int smb_direct_rdma_write(struct ksmbd_transport *t, void *buf,
  ...
  return smb_direct_rdma_xmit(smb_trans_direct_transfort(t), buf, buflen, desc,
                              desc_len, false);
}
...
static int smb_direct_send_negotiate_response(struct smb_direct_transport *t,
  ...
  if (failed) {
    ...
    resp->reserved = 0;
    resp->credits_requested = cpu_to_le16(t->send_credit_target);
    ...
    resp->max_fragmented_size = cpu_to_le32(t->max_fragmented_recv_size);
  }

  sendmsg->sge[0].addr = ib_dma_map_single(t->cm_id->device, (void *)resp,
                                           sizeof(*resp), DMA_TO_DEVICE);
  ret = ib_dma_mapping_error(t->cm_id->device, sendmsg->sge[0].addr);
  if (ret) {
    smb_direct_free_sendmsg(t, sendmsg);
    return ret;
...
static int smb_direct_accept_client(struct smb_direct_transport *t) {
  ...
  t->cm_id->device->ops.get_port_immutable(t->cm_id->device, t->cm_id->port_num,
                                           &port_immutable);
...
static unsigned int
smb_direct_get_max_fr_pages(struct smb_direct_transport *t) {
  return min_t(unsigned int, t->cm_id->device->attrs.max_fast_reg_page_list_len,
               256);
}
...
static int smb_direct_init_params(struct smb_direct_transport *t,
  ...
  if (max_send_wrs > dev_attr.max_cqe || max_send_wrs > dev_attr.max_qp_wr) {
    pr_err("consider lowering send_credit_target = %d\n",
           smb_direct_send_credit_target);
    pr_err("Possible CQE overrun, device reporting max_cqe %d max_qp_wr %d\n",
           dev_attr.max_cqe, dev_attr.max_qp_wr);
    return -EINVAL;
  }

  if (smb_direct_receive_credit_max > dev_attr.max_cqe ||
      smb_direct_receive_credit_max > dev_attr.max_qp_wr) {
  ...
  if (dev_attr.max_send_sge < SMB_DIRECT_MAX_SEND_SGES) {
    pr_err("warning: device max_send_sge = %d too small\n",
           dev_attr.max_send_sge);
    return -EINVAL;
  }
  if (dev_attr.max_recv_sge < SMB_DIRECT_MAX_RECV_SGES) {
...
static int smb_direct_create_pools(struct smb_direct_transport *t) {
  ...
  t->sendmsg_mempool = mempool_create(t->send_credit_target, mempool_alloc_slab,
                                      mempool_free_slab, t->sendmsg_cache);
  if (!t->sendmsg_mempool)
    goto err;

  snprintf(name, sizeof(name), "smb_direct_resp_%p", t);
  t->recvmsg_cache = kmem_cache_create(
      name, sizeof(struct smb_direct_recvmsg) + t->max_recv_size, 0,
      SLAB_HWCACHE_ALIGN, NULL);
  if (!t->recvmsg_cache)
    goto err;

  t->recvmsg_mempool = mempool_create(t->recv_credit_max, mempool_alloc_slab,
...
static int smb_direct_create_qpair(struct smb_direct_transport *t,
  ...
  t->recv_cq = ib_alloc_cq(t->cm_id->device, t, t->recv_credit_max, 0,
                           IB_POLL_WORKQUEUE);
  if (IS_ERR(t->recv_cq)) {
...
#endif
    ret = ib_mr_pool_init(t->qp, &t->qp->rdma_mrs, t->max_rw_credits,
                          IB_MR_TYPE_MEM_REG, t->pages_per_rw_credit, 0);
    if (ret) {
...
static int smb_direct_prepare(struct ksmbd_transport *t) {
  ...
  ret = wait_event_interruptible_timeout(
      st->wait_status,
      st->negotiation_requested || st->status == SMB_DIRECT_CS_DISCONNECTED,
  ...
  st->max_recv_size =
      min_t(int, st->max_recv_size, le32_to_cpu(req->preferred_send_size));
  st->max_send_size =
      min_t(int, st->max_send_size, le32_to_cpu(req->max_receive_size));
  st->max_fragmented_send_size = le32_to_cpu(req->max_fragmented_size);
...
static int smb_direct_handle_connect_request(struct rdma_cm_id *new_cm_id) {
  ...
  handler = kthread_run(ksmbd_conn_handler_loop, KSMBD_TRANS(t)->conn,
                        "ksmbd:r%u", smb_direct_port);
  if (IS_ERR(handler)) {
    ret = PTR_ERR(handler);
...
#else
  smb_direct_wq =
      alloc_workqueue("ksmbd-smb_direct-wq", WQ_HIGHPRI | WQ_MEM_RECLAIM, 0);
#endif
  if (!smb_direct_wq)
    return -ENOMEM;
...
static bool ksmbd_find_rdma_capable_netdev(struct net_device *netdev) {
  ...
  list_for_each_entry(smb_dev, &smb_direct_device_list, list) {
    for (i = 0; i < smb_dev->ib_dev->phys_port_cnt; i++) {
      ...
      ndev = smb_dev->ib_dev->ops.get_netdev(smb_dev->ib_dev, i + 1);
      if (!ndev)
        continue;
...
bool ksmbd_rdma_capable_netdev(struct net_device *netdev) {
  ...
  if (netif_is_bridge_master(netdev) || netdev->priv_flags & IFF_802_1Q_VLAN)
    netdev_for_each_lower_dev(
        netdev, lower_dev,
        iter) if (ksmbd_find_rdma_capable_netdev(lower_dev)) return true;
